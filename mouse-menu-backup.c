...
static yed_event_handler h_left_click;
static int               on_word;
...
/* Event Handlers */
static void left_click_handler(yed_event* event);
...
int yed_plugin_boot(yed_plugin *self) {
    ...
    h_left_click.kind = EVENT_KEY_PRESSED;
    h_left_click.fn   = left_click_handler;
...
/*     create the menu for selection */
    ...
    do_popup();
    yed_plugin_add_event_handler(Self, h_left_click);
...
static void left_click_handler(yed_event *event) {
    ...
    ||  MOUSE_BUTTON(event->key) == MOUSE_BUTTON_MIDDLE
    ||  MOUSE_KIND(event->key)   == MOUSE_WHEEL_UP
    ||  MOUSE_KIND(event->key)   == MOUSE_WHEEL_DOWN) {
    ...
    if (MOUSE_KIND(event->key) == MOUSE_RELEASE) {
        if (MOUSE_BUTTON(event->key) == MOUSE_BUTTON_LEFT) {
            ...
            yed_delete_event_handler(h_left_click);
...
static void key_handler(yed_event *event) {
    ...
    if(popup.is_up) {
        ...
        &&  MOUSE_KIND(event->key) == MOUSE_RELEASE) {
